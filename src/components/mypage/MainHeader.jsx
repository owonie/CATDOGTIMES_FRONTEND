      ...
      <div id='main_header' className='main_header pt-3 pb-5'>
        <div className='profile d-flex'>
          <div className='profile_photo mr-5'>
            {users !== null ? (
              <img
                src={`${imgPath}${users.memberPhoto}`}
                alt={users.memberNickname}
                className='img-thumbnail rounded-circle'
              />
            ) : (
              <img
                src={`${imgPath}undefined.jpg`}
                alt='undefiend'
                className='img-thumbnail rounded-circle'
              />
            )}
          ...
          <div className='profile_info d-flex flex-column'>
            ...
            <div className='summ_cnt'>
              ...
              <span className='follower' onClick={followers}>
                팔로워 (
                <span>{users !== null ? users.followerCnt : 'NoData'}</span>)
              </span>
              <span className='following' onClick={following}>
                팔로잉 (
                <span>{users !== null ? users.followingCnt : 'NoData'}</span>)
              </span>
            </div>
            <p className='accountid'>
              @{users !== null ? users.memberId : 'NoData'}
            </p>
          </div>
        </div>
        ...
        {followlist === null ? (
          ...
          <Modal open={open} onOk={handleOk} onCancel={handleCancel}>
            <h3>
              {followlist && followlist.length > 0 ? followlist[0].type : ''}
            </h3>
              ...
              {followlist && followlist.length > 0
                ? followlist.map((da, i) => (
                    <li key={i} className='d-flex'>
                      ...
                      <button
                        onClick={() =>
                          deleteFollow(da.followId, followlist[0].type)
                        }
                        className='delete btn-danger'
                      >
                        {followlist[0].type === 'follower'
                          ? '팔로워 차단'
                          : '팔로잉 취소'}
                      </button>
                    </li>
